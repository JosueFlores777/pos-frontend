...
import { CTable, CRow, CCol, CContainer } from '@coreui/react'
...
const Index = (props) => {
  ...
  const creardata = (data) => {
    ...
    data.valores.forEach((ele) => {
      ...
      props.definicion.forEach((element) => {
        linea.push(<td key={linea.length}>{getval(element, ele)}</td>);
      });
      info.push(<tr key={info.length}>{linea}</tr>);
    });
  ...
  if (loading) {
    ...
    return (
      <div>
        <CTable borderless striped responsive size="md">
          <thead>
            <tr>{state.encabezado}</tr>
          </thead>

          <tbody>{state.cuerpo}</tbody>
        </CTable>
